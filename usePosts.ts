...
export function usePosts(tab: FeedTab) {
  ...
  const fetchPosts = async (): Promise<Post[]> => {
    let query = supabase
      ...
      .select(`
        *,
        author:profiles!posts_user_id_fkey(id, name, avatar_url, user_type, is_bot, eliminated_at, premium_active)
      `)
    ...
    mutationFn: async ({ content, imageUrl, videoUrl, musicUrl, additionalImages, categoria }: {
      ...
      const { data, error } = await supabase
        ...
        .insert({
          ...
          tema_id: profile?.tema_id || null,
          multiplicador: profile?.multiplicador_ativo || 1,
          ...(categoria ? { categoria } : {}),
        } as any)
        .select()
        .single();
