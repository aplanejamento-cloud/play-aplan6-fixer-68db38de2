...
const FREE_MUSIC = [
  { name: "Vibe Tropical", url: "https://www.soundhelix.com/examples/mp3/SoundHelix-Song-1.mp3" },
  { name: "Noite Urbana", url: "https://www.soundhelix.com/examples/mp3/SoundHelix-Song-2.mp3" },
  { name: "Energia Pura", url: "https://www.soundhelix.com/examples/mp3/SoundHelix-Song-3.mp3" },
];
...
const UnifiedEditorModal = ({
  ...
  const [imageOverlays, setImageOverlays] = useState<{ id: string; url: string; x: number; y: number; size: number }[]>([]);
  ...
  const handleOverlayUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setImageOverlays(prev => [...prev, { id: Date.now().toString(), url, x: 50, y: 50, size: 100 }]);
    e.target.value = "";
    toast.success("Foto sobreposta adicionada! 🖼️");
  };
  ...
  const handlePointerMove = useCallback((e: React.PointerEvent) => {
    ...
    else if (dragging.type === "emoji") setEmojiOverlays(prev => prev.map(em => em.id === dragging.id ? { ...em, x, y } : em));
    else if ((dragging.type as any) === "image") setImageOverlays(prev => prev.map(ov => ov.id === dragging.id ? { ...ov, x, y } : ov));
  }, [dragging]);
  ...
  const removeOverlay = (type: "text" | "emoji" | "image", id: string) => {
    ...
    else setImageOverlays(prev => prev.filter(ov => ov.id !== id));
  };
    ...
    return new Promise((resolve, reject) => {
      ...
      img.onload = () => {
        ...
        imageOverlays.forEach(ov => {
          ...
          const size = (ov.size / 400) * canvas.width;
          ctx.drawImage(ovImg, px - size / 2, py - size / 2, size, size);
        });
        ...
        {!image ? (
          ...
          <>
            ...
            <div className="flex gap-1.5 flex-wrap">
              ...
              {!isAvatar && (
                <>
                  ...
                  <Button variant="outline" size="sm" onClick={() => overlayInputRef.current?.click()} className="text-xs h-8">
                    <ImageIcon className="w-3.5 h-3.5 mr-1" />Sobrepor
                  </Button>
            ...
            <div className="space-y-2">
              <p className="text-[10px] text-muted-foreground font-bold uppercase">Músicas Free</p>
                ...
                {FREE_MUSIC.map(m => (
                  <button
                    ...
                    onClick={() => {
                      setMusicFile(null);
                      setMusicUrl(m.url);
                      toast.success(`🎵 Música: ${m.name}`);
                    }}
                    className={cn(
                      "px-2 py-1 rounded text-[10px] border transition-all",
                      musicUrl === m.url ? "bg-primary/20 border-primary text-primary" : "bg-muted/30 border-border text-muted-foreground hover:border-primary/40"
                    )}
                  >
                    {m.name}
                  </button>
            ...
            {musicUrl && (
              <div className="flex items-center gap-2 bg-card p-2 rounded-lg border border-border">
                ...
                <button onClick={() => { setMusicFile(null); if (musicUrl && !FREE_MUSIC.some(m => m.url === musicUrl)) URL.revokeObjectURL(musicUrl); setMusicUrl(null); }} className="p-1">
            ...
            >
              ...
              {imageOverlays.map(ov => (
                <div key={ov.id} className="absolute cursor-move" style={{ left: `${ov.x}%`, top: `${ov.y}%`, transform: "translate(-50%,-50%)", width: ov.size, userSelect: "none" }} onPointerDown={() => handlePointerDown("image" as any, ov.id)}>
                  <img src={ov.url} alt="Overlay" className="w-full h-auto pointer-events-none" />
                  <button onClick={(ev) => { ev.stopPropagation(); removeOverlay("image", ov.id); }} className="absolute -top-2 -right-2 w-5 h-5 bg-destructive rounded-full flex items-center justify-center"><X className="w-3 h-3 text-white" /></button>
                </div>
              ))}
              {/* Avatar circle guide */}
              {isAvatar && (
