...
const ChatPage = () => {
  ...
  const dataFim = chat?.data_fim ? new Date(chat.data_fim) : null;
  const isExpired = dataFim ? isPast(dataFim) : false;
  const isActive = chat?.ativa && !isExpired && dataFim !== null;
                ...
                <div key={msg.id} className={cn("flex gap-2", isMine ? "flex-row-reverse" : "flex-row")}>
                  ...
                  <div className={cn("max-w-[75%] space-y-1", isMine ? "items-end" : "items-start")}>
                    ...
                    <time className={cn("text-[10px] text-muted-foreground", isMine ? "text-right" : "text-left")}>
                      {msg.created_at ? formatDistanceToNow(new Date(msg.created_at), { addSuffix: true, locale: ptBR }) : "agora"}
                    </time>
                  </div>
                </div>
              );
