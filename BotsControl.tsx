    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="container mx-auto px-4 py-6 max-w-4xl space-y-6">
        ...
        <div className="grid grid-cols-2 gap-3">
          <Button
            onClick={async () => {
              toast.info("Iniciando reconfiguração automática...");
              await triggerVisualLikes.mutateAsync();
              await triggerPost.mutateAsync();
              await triggerInteract.mutateAsync();
              await assignAvatars.mutateAsync();
              toast.success("Bots reconfigurados com sucesso! 🤖✅");
            }}
            disabled={triggerPost.isPending || triggerVisualLikes.isPending || triggerInteract.isPending || assignAvatars.isPending}
            className="col-span-2 bg-primary text-primary-foreground font-bold h-auto py-4 shadow-gold animate-pulse-gold"
          >
            <RefreshCw className={cn("w-6 h-6 mr-2", (triggerPost.isPending || triggerVisualLikes.isPending || triggerInteract.isPending || assignAvatars.isPending) && "animate-spin")} />
            RECONFIGURAR TUDO AUTOMÁTICO
          </Button>
          <Button
