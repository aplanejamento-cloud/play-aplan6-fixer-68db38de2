...
import { Loader2, ThumbsUp, ThumbsDown, Trash2, Image, Video, Music, X, Gavel, ChevronDown, ChevronUp, MessageCircle } from "lucide-react";
...
// ─── Reply Form (inline, for all authenticated users) ─────────────────────
const ReplyForm = ({ postId, parentId, onClose }: { postId: string; parentId?: string; onClose: () => void }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!texto.trim() && !media) { toast.error("Escreva algo!"); return; }
    ...
    try {
      ...
      await createComment.mutateAsync({ postId, texto: texto.trim(), midiaUrl, midiaType, parentId });
      setTexto("");
      removeMedia();
      onClose();
      toast.success("Resposta publicada!");
    } catch { toast.error("Erro ao publicar resposta"); }
  };

  const isSubmitting = createComment.isPending || uploading;

  return (
    <form onSubmit={handleSubmit} className={cn("mt-2 mb-1", parentId ? "ml-8" : "ml-12")}>
      {isJuiz && (
        <>
      ...
      <div className="flex gap-2">
        <div className="w-7 h-7 rounded-full bg-accent/20 flex items-center justify-center overflow-hidden flex-shrink-0 ring-1 ring-accent/30">
          {profile.avatar_url ? (
        ...
        <div className="flex-1 space-y-2">
          <Textarea
            placeholder={isJuiz ? "Responder como juiz... ⚖️" : "Responder..."}
            value={texto}
            onChange={(e) => setTexto(e.target.value)}
            className="min-h-[50px] text-sm bg-background border-border resize-none"
            maxLength={300}
          ...
          {media && isJuiz && (
            <div className="relative rounded-lg overflow-hidden border border-border bg-muted/30">
              <button type="button" onClick={removeMedia} className="absolute top-1 right-1 z-10 bg-background/80 rounded-full p-0.5">
                <X className="w-3 h-3 text-foreground" />
              </button>
              {media.type === "image" && <img src={media.url} alt="" className="w-full max-h-32 object-cover" />}
              {media.type === "video" && <video src={media.url} controls className="w-full max-h-32" />}
              {media.type === "music" && (
          ...
          <div className="flex items-center justify-between">
            <div className="flex gap-1">
              ...
              <button type="button" onClick={onClose} className="p-1 text-xs text-muted-foreground hover:text-foreground">
                Cancelar
              </button>
            </div>
            <Button type="submit" size="sm" disabled={isSubmitting || (!texto.trim() && !media)}
              className="h-6 px-2 text-[11px] bg-accent hover:bg-accent/90 text-accent-foreground">
              {isSubmitting ? <Loader2 className="w-3 h-3 animate-spin" /> : "Responder"}
            </Button>
          </div>
        </div>
      </div>
...
// ─── Single Comment Card ───────────────────────────────────────────────────
const CommentCard = ({ comment, postId, hasThread, allComments }: { comment: Comment; postId: string; hasThread: boolean; allComments: Comment[] }) => {
  ...
  return (
    <div className={cn(comment.parent_id && "ml-8 border-l border-accent/10 pl-3")}>
      <div className="flex gap-2.5 group">
        {/* Avatar */}
        <div className={cn(
          "rounded-full bg-accent/20 flex items-center justify-center overflow-hidden flex-shrink-0 ring-1 ring-accent/30 mt-0.5",
          comment.parent_id ? "w-8 h-8" : "w-10 h-10"
        )}>
          {comment.juiz?.avatar_url ? (
            <img src={comment.juiz.avatar_url} alt={comment.juiz.name} className="w-full h-full object-cover" />
          ) : (
            <span className={cn("text-accent font-bold", comment.parent_id ? "text-[10px]" : "text-xs")}>
              {comment.juiz?.name?.charAt(0).toUpperCase()}
            </span>
          )}
        </div>
        ...
        <div className="flex-1 min-w-0">
          <div className="bg-muted/30 rounded-xl px-3 py-2.5 border border-accent/20">
            ...
            <div className="flex items-center justify-between mb-1 gap-2">
              <div className="flex items-center gap-1.5">
                ...
                <span className="text-xs font-semibold text-accent truncate">{comment.juiz?.name || "Usuário"}</span>
              </div>
          ...
          <div className="flex items-center gap-3 mt-1 px-1">
            ...
            >
              <ThumbsUp className={cn("w-3.5 h-3.5", comment.myReaction === "like" && "fill-current")} />
            ...
            >
              <ThumbsDown className={cn("w-3.5 h-3.5", comment.myReaction === "bomba" && "fill-current")} />
              <span className="font-bold">{comment.bombas}</span>
            </button>

            {/* Responder button - visible to all authenticated users when thread exists */}
            {user && hasThread && !comment.parent_id && (
              <button
      ...
      {/* Inline reply form */}
      {showReply && (
        <ReplyForm postId={postId} parentId={comment.id} onClose={() => setShowReply(false)} />
      )}
      ...
      {replies.length > 0 && (
        <div className="mt-3 space-y-3">
          {replies.map(reply => (
            <CommentCard key={reply.id} comment={reply} postId={postId} hasThread={hasThread} allComments={allComments} />
          ))}
        </div>
      )}
    </div>
...
// ─── Create Comment Form (apenas juízes - para INICIAR conversa) ──────────
const InitialCommentForm = ({ postId }: { postId: string }) => {
  const { profile } = useAuth();
  const createComment = useCreateComment();
  const { upload, uploading, ACCEPT } = useMediaUpload();
...
const CommentSection = ({ postId }: CommentSectionProps) => {
  ...
  // Jogador sees nothing if no comments exist (no thread started)
  // Juiz always sees the "Comentar" form to start a thread
  if (!hasComments && !isJuiz) return null;

  const rootComments = comments.filter(c => !c.parent_id);
  const PREVIEW_COUNT = 2;
  const showAll = expanded || rootComments.length <= PREVIEW_COUNT;
  const visibleComments = showAll ? rootComments : rootComments.slice(0, PREVIEW_COUNT);
  ...
  return (
    <div className="space-y-3">
      ...
      {!isLoading && (
        <div className="space-y-3">
          {visibleComments.map((comment) => (
            <CommentCard key={comment.id} comment={comment} postId={postId} hasThread={hasComments} allComments={comments} />
          ))}

          {/* Show more / less toggle */}
          {rootComments.length > PREVIEW_COUNT && (
            <button
      ...
      {/* Form para juízes INICIAREM conversa (sempre visível para juiz) */}
      <InitialCommentForm postId={postId} />
    </div>
  );
};
