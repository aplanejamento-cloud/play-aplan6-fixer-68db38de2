...
const DonationForm = () => {
  ...
  const doarMutation = useMutation({
    mutationFn: async () => {
      ...
      if (!whatsapp.trim() || whatsapp.length < 10) throw new Error("WhatsApp válido é obrigatório (DDD + número)");
    ...
    <Card className="p-4 space-y-4 border-primary/20">
      ...
      <div>
        <Input 
          ...
          className={whatsapp && whatsapp.length < 10 ? "border-destructive" : !whatsapp.trim() ? "border-destructive/50" : ""} 
        />
        {whatsapp && whatsapp.length < 10 && (
          <p className="text-xs text-destructive mt-1">WhatsApp deve ter DDD + número (mínimo 10 dígitos)</p>
        )}
      </div>
      ...
      <Button
        ...
        disabled={!selectedFile || !whatsapp.trim() || whatsapp.length < 10 || doarMutation.isPending || uploading}
