...
export function useDuels() {
  ...
  const challengeUser = async (challengedId: string, stakeAmount: number = 100, duelType: "normal" | "fatalite" = "normal") => {
    ...
    if (profile.user_type === "juiz") {
      toast.error("Juízes não podem participar de duelos!");
      return;
    }
    ...
    const { data: challengerDuels } = await supabase
      .from("duels")
      .select("id")
      .in("status", ["pending", "active"])
      .or(`challenger_id.eq.${user.id},challenged_id.eq.${user.id}`);

    if (challengerDuels && challengerDuels.length > 0) {
      toast.error("Você já está em um duelo (pendente ou ativo)!");
      return;
    }

    // Check if challenged is already in a duel
    const { data: challengedDuels } = await supabase
      .from("duels")
      .select("id")
      .in("status", ["pending", "active"])
      .or(`challenger_id.eq.${challengedId},challenged_id.eq.${challengedId}`);

    if (challengedDuels && challengedDuels.length > 0) {
      const challengedName = duels.find(d => d.challenged_id === challengedId || d.challenger_id === challengedId)?.challenged_profile?.name || "O jogador";
      toast.error(`${challengedName} já está duelando! Escolha outro`, {
        style: { background: '#ef4444', color: '#fff' }
      });
      console.log(`[Duels] Bloqueio: ${challengedName} já está em um duelo ativo.`);
      
      // Send notification to challenger about the busy player
      await supabase.from("notifications").insert({
        user_id: user.id,
        tipo: "duelo",
        mensagem: `⚔️ ${challengedName} já está duelando! Escolha outro`,
      });
      return;
    }
