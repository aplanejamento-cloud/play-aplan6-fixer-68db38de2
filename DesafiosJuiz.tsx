                  ...
                  <div className="space-y-2">
                    <input
                      ...
                      onChange={async (e) => {
                        ...
                        const toastId = toast.loading("Enviando prova...");
                        const url = await upload(file, file.type.startsWith("video") ? "video" : "image");
                        ...
                        const proofText = `Fiz o desafio do Juiz ${d.juiz_profile?.name}! 🎬\n\n"${d.texto}"`;
                        const { error } = await supabase.from("posts").insert({
                          user_id: user?.id,
                          content: proofText,
                          tipo: "normal",
                          image_url: file.type.startsWith("image") ? url : null,
                          video_url: file.type.startsWith("video") ? url : null,
                          expires_at: new Date(Date.now() + 24 * 60 * 60 * 1000).toISOString(),
                        } as any);
                        
                        if (error) {
                          toast.error("Erro ao publicar prova", { id: toastId });
                        } else {
                          toast.success("Prova publicada no feed! 🚀", { id: toastId });
                        }
                      }}
                    />
                    <Button
                      onClick={() => document.getElementById(`proof-input-${d.id}`)?.click()}
                      disabled={uploading}
                      className="w-full bg-accent text-accent-foreground hover:bg-accent/90"
                    >
                      <CheckCircle className="w-4 h-4 mr-2" /> {uploading ? "Enviando..." : "Fiz isso! (Postar Prova)"}
                    </Button>
                  </div>
                )}
              </CardContent>
