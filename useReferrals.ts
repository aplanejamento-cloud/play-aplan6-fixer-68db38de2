  ...
  const claimNetwork = async (network: string) => {
    ...
    if (existing) {
      ...
      if (!error) {
        ...
        const currentLikes = await getCurrentLikes();
        await supabase
          .from("profiles")
          .update({ total_likes: currentLikes + 10 })
          .eq("user_id", user.id);
    ...
    if (!error) {
      ...
      await supabase
        .from("profiles")
        .update({ total_likes: currentLikes + 100 })
        .eq("user_id", user.id);
      await refreshProfile();
      await fetchReferrals();
      return { success: true, likes: 100 };
    }
    return { success: false, likes: 0 };
  };
