  ...
  return useMutation({
    mutationFn: async ({
      ...
      if (currentReaction === reactionType) {
        ...
        const { data: current } = await (supabase.from("comments" as any).select("likes, bombas").eq("id", commentId).single() as any);
        if (current) {
          const updateData: any = {};
          if (reactionType === "like") updateData.likes = Math.max(0, (current.likes || 0) - 1);
          else updateData.bombas = Math.max(0, (current.bombas || 0) - 1);
          await supabase.from("comments" as any).update(updateData).eq("id", commentId);
        }
        return;
      }
      ...
      if (current) {
        ...
        if (reactionType === "like") {
          updateData.likes = (current.likes || 0) + 1;
          if (currentReaction === "bomba") updateData.bombas = Math.max(0, (current.bombas || 0) - 1);
        } else {
          updateData.bombas = (current.bombas || 0) + 1;
          if (currentReaction === "like") updateData.likes = Math.max(0, (current.likes || 0) - 1);
        }
        await supabase.from("comments" as any).update(updateData).eq("id", commentId);
      }
    },
