...
const Duels = () => {
  ...
  const filteredDuels = duels.filter(duel => {
    ...
    const challengerName = duel.challenger_profile?.name?.toLowerCase() || "";
    const challengedName = duel.challenged_profile?.name?.toLowerCase() || "";
    return challengerName.includes(search) || challengedName.includes(search);
  });

  useEffect(() => {
    console.log(`[Duels] Feed filtrado por: "${feedSearch}". Encontrados: ${filteredDuels.length} duelos.`);
  }, [feedSearch, filteredDuels.length]);
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <main className="container mx-auto px-4 py-6 max-w-2xl space-y-6">
        ...
        {/* Search & Challenge - only for jogadores */}
        ...
        <div className="relative">
          <Input
            placeholder="Filtrar duelos por nome do jogador..."
            value={feedSearch}
            onChange={(e) => setFeedSearch(e.target.value)}
            className="bg-card border-border pr-10"
          />
          <Swords className="absolute right-3 top-1/2 -translate-y-1/2 w-4 h-4 text-muted-foreground" />
        </div>
        ...
        <div className="space-y-6">
          <div className="space-y-4">
            <h3 className="font-cinzel text-sm text-primary flex items-center gap-2">
              <Swords className="w-4 h-4" /> Duelos Ativos
            </h3>

            {isLoading ? (
              [...Array(2)].map((_, i) => <Skeleton key={i} className="h-40 rounded-xl" />)
            ) : filteredDuels.filter(d => d.status === "active").length === 0 ? (
              <div className="text-center py-8 text-muted-foreground text-xs border border-dashed border-border rounded-xl">
                {feedSearch ? "Nenhum duelo ativo encontrado para esta busca." : "Nenhum duelo ativo no momento. ⚔️"}
              </div>
            ) : (
              filteredDuels.filter(d => d.status === "active").map((duel) => (
          ...
          <div className="space-y-4">
            <h3 className="font-cinzel text-sm text-muted-foreground flex items-center gap-2">
              <CheckCircle className="w-4 h-4" /> Duelos Encerrados
            </h3>

            {isLoading ? (
              [...Array(2)].map((_, i) => <Skeleton key={i} className="h-40 rounded-xl" />)
            ) : filteredDuels.filter(d => d.status === "completed" || d.status === "refused").length === 0 ? (
              <div className="text-center py-8 text-muted-foreground text-xs border border-dashed border-border rounded-xl">
                {feedSearch ? "Nenhum duelo encerrado encontrado para esta busca." : "Nenhum duelo encerrado ainda."}
              </div>
            ) : (
              filteredDuels.filter(d => d.status === "completed" || d.status === "refused").map((duel) => (
                <DuelCard key={duel.id} duel={duel} />
              ))
            )}
          </div>
  ...
  return (
    <Card className="bg-card/80 border-border overflow-hidden">
      <CardContent className="py-4 space-y-3">
        ...
        {isAdmin && duel.status === "active" && (
          <div className="pt-2">
            <Button size="sm" variant="destructive" className="w-full" onClick={() => resolveDuel(duel.id)}>
              <CheckCircle className="w-4 h-4 mr-1" /> Admin: Encerrar Duelo
            </Button>
          </div>
        )}
      </CardContent>
