...
const ReplyForm = ({ postId, onClose }: { postId: string; onClose: () => void }) => {
  ...
  return (
    <form onSubmit={handleSubmit} className="ml-12 mt-2 mb-1">
      {isJuiz && (
        <>
...
// ─── Single Comment Card ───────────────────────────────────────────────────
const CommentCard = ({ comment, postId, hasThread }: { comment: Comment; postId: string; hasThread: boolean }) => {
  ...
  return (
    <div>
      ...
      {/* Inline reply form */}
      {showReply && (
        <ReplyForm postId={postId} onClose={() => setShowReply(false)} />
      )}
    </div>
  );
};
      ...
      {!isLoading && (
        <div className="space-y-3">
          {visibleComments.map((comment) => (
            <CommentCard key={comment.id} comment={comment} postId={postId} hasThread={hasComments} />
          ))}
