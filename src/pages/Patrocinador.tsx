...
import { Crown, MessageCircle, TrendingUp, Users, Heart, Save, Loader2 } from "lucide-react";
...
const Patrocinador = () => {
  ...
  const { data: patrocinio } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await supabase.from("patrocinio" as any).select("id, whatsapp").limit(1).maybeSingle();
      return data as any;
    },
  });

  const whatsappFromDb = patrocinio?.whatsapp || "(11) 99999-9999";
  const whatsappClean = whatsappFromDb.replace(/\D/g, "");
  ...
  useEffect(() => {
    if (patrocinio?.whatsapp) setEditWhatsapp(patrocinio.whatsapp);
  }, [patrocinio?.whatsapp]);
  ...
  const handleSaveWhatsapp = async () => {
    ...
    try {
      if (patrocinio?.id) {
        await supabase.from("patrocinio" as any).update({ whatsapp: editWhatsapp }).eq("id", patrocinio.id);
      } else {
        await supabase.from("patrocinio" as any).insert({ whatsapp: editWhatsapp });
      }
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <main className="container mx-auto px-4 py-8 max-w-2xl space-y-8">
        ...
        {isAdmin && (
          <Card className="border-primary/30 bg-primary/5">
            <CardContent className="py-4 space-y-2">
              <p className="text-xs font-bold text-primary">⚙️ Admin — WhatsApp Patrocínio</p>
              <div className="flex gap-2">
                <Input
                  ...
                  type="tel"
                />
                <Button size="sm" onClick={handleSaveWhatsapp} disabled={saving}>
        ...
        <section className="text-center space-y-4">
          <a
            href={`https://wa.me/55${whatsappClean}?text=${WHATSAPP_MSG}`}
            target="_blank"
          ...
          <p className="text-xs text-muted-foreground">WhatsApp: {whatsappFromDb}</p>
        </section>
