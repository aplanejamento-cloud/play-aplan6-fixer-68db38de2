  ...
  const handleGerarPix = async () => {
    ...
    try {
      const { data, error } = await supabase.functions.invoke("create-pix-payment", {
        body: { tipo: "premium", valor: 9.9, likes: 1000, usuario_id: user.id },
      });
  ...
  const startPolling = (compraId: string) => {
    ...
    const interval = setInterval(async () => {
      ...
      if (data?.status === "aprovado") {
        ...
        toast.success("✅ PIX detectado! +1000 likes + Cadastro Premium ativo!");
        setPixData(null);
        onOpenChange(false);
      }
    }, 5000);
      ...
      <DialogContent className="bg-card border-border max-w-md">
        ...
        <div className="flex flex-col items-center gap-4 mt-4">
          <div className="bg-primary/10 border border-primary/30 rounded-xl p-4 text-center">
            <p className="font-cinzel text-3xl font-bold text-primary">R$ 9,90</p>
