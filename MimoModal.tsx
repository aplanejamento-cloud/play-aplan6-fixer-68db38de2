...
const MimoModal = ({ open, onOpenChange, targetUserId, targetName }: MimoModalProps) => {
  ...
  const handleSend = async () => {
    ...
    try {
      // Check if target is a jogador or juiz (judges can receive mimos from other judges)
      ...
      const isSenderJuiz = profile?.user_type === "juiz";
      const isTargetJuiz = targetProfile?.user_type === "juiz";
      const isTargetJogador = targetProfile?.user_type === "jogador";

      if (!isTargetJogador && !(isSenderJuiz && isTargetJuiz)) {
        toast.error("Mimos só podem ser enviados para jogadores ou entre juízes!");
        return;
      }
      ...
      toast.error(error.message || "Erro ao enviar mimo");
    }
  };
